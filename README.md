...
Желательно иметь сразу два акканута в телеграм, один пользовательский, другой операторский, который создал чат бота.
 Сценарий работы пользователя в ТГ боте
 ...
 Каждый ответ оператора обозначен строкой "Ответ оператора:" чтобы пользователь понимал, что говорит с живым человеком.
   ...
   После того как разговор завершен клиент снова нажимает кнопку , только в этот раз кнопка уже имеет вид "Отсоединить оператора"
...
Так же была идея сделать так чтобы оператор полчил полную истори переписки пользователя с ботом, для получения большей информации.
